...
class database
{
    ...
    public function editProduk($data)
    {
        if (!empty($data["gambar"])) {
            ...
            $gambar = $data["gambarBefore"];
        }
        mysqli_query($this->koneksi, "UPDATE tabel_produk SET produk = '" . $data["produk"] . "', jenis = '" . $data["jenis"] . "', harga = '" . $data["harga"] . "', gambar = '$gambar' WHERE id = '" . $data["id"] . "' ");
    }

    public function tambahProduk($data)
    {
        mysqli_query($this->koneksi, "INSERT INTO tabel_produk VALUES ('', '" . $data["produk"] . "', '" . $data["jenis"] . "', '" . $data["harga"] . "', '" . $data["gambar"] . "', 'ENABLE')");
    }
...
FROM `tabel_transaksi` 
    ...
    public function searchOperator($username)
    {
        ...
        $id = $row["id"];
        $query = "
        SELECT `tabel_transaksi`.`id`, `tabel_transaksi`.`id_user`, `tabel_transaksi`.`id_operator`, `tabel_transaksi`.`diskon`, `tabel_transaksi`.`total`, `tabel_transaksi`.`bayar`, `tabel_transaksi`.`tanggal`, `tabel_transaksi`.`status` FROM `tabel_transaksi` WHERE tabel_transaksi.id_operator LIKE '$id%' AND tabel_transaksi.status = 'DONE'";
